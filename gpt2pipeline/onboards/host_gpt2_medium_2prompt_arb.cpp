...
constexpr int kUsefulPrompts = 2;
...
constexpr size_t kC0KvSliceWords = 16 * 128 * 2;
constexpr size_t kC0KvTSliceWords = 16 * 64 * 4;
constexpr size_t kC0KvSlices = kNumLayers * kNumPrompts;
constexpr size_t kC0KvBytes = kC0KvSliceWords * kC0KvSlices * kWordBytes;
...
struct DeviceBundle {
  ...
  NamedBo kv_v_hbm;
  NamedBo kv_k_t_hbm;
  std::vector<uint8_t> seed_hidden_shadow;
  std::vector<uint8_t> final_hidden_shadow;
  PackedC0 c0;
  PackedC1 c1;
  PackedC2 c2;
};
...
NamedBo make_bo(xrt::device& device, xrt::kernel& kernel, int arg_index,
  ...
  return NamedBo{
      name,
      bytes,
      xrt::bo(device, bytes, xrt::bo::flags::device_only, kernel.group_id(arg_index)),
  };
}
...
void fill_bo_pattern(NamedBo& buffer, uint32_t seed) {
  std::vector<uint8_t> host(buffer.bytes);
  uint32_t state = seed;
  for (size_t i = 0; i < buffer.bytes; ++i) {
    state = state * 1664525U + 1013904223U;
    host[i] = static_cast<uint8_t>(state >> 24);
  }
  buffer.bo.write(host.data(), host.size(), 0);
}

void clear_bo(NamedBo& buffer) {
  std::vector<uint8_t> host(buffer.bytes, 0);
  buffer.bo.write(host.data(), host.size(), 0);
}

void write_seed_slot_pattern(std::vector<uint8_t>& seed_shadow, int prompt_slot, int token_round,
                             int cache_position) {
  ...
  for (int i = 0; i < kHidden; ++i) {
    ...
    seed_shadow[slot_offset + static_cast<size_t>(i)] = static_cast<uint8_t>(state >> 24);
  }
}

uint64_t checksum_final_slot(const std::vector<uint8_t>& final_shadow, int prompt_slot) {
  if (prompt_slot < 0 || prompt_slot >= kNumPrompts)
    throw std::runtime_error("prompt slot out of range");
  ...
  for (int i = 0; i < kHidden; ++i) {
    acc ^= static_cast<uint64_t>(final_shadow[slot_offset + static_cast<size_t>(i)]);
...
DeviceBundle make_device_bundle(const std::string& xclbin_path, unsigned device_index) {
  ...
  d.kv_k_hbm = make_bo(d.device, d.k_c0, 17, kC0KvBytes, "c0_kv_k_hbm");
  d.kv_v_hbm = make_bo(d.device, d.k_c0, 18, kC0KvBytes, "c0_kv_v_hbm");
  d.kv_k_t_hbm = make_bo(d.device, d.k_c0, 19, kC0KvTBytes, "c0_kv_k_t_hbm");
  d.seed_hidden_shadow.assign(kSeedBoBytes, 0);
  d.final_hidden_shadow.assign(kFinalBoBytes, 0);
  ...
  for (int i = 0; i < 8; ++i) {
    auto& bank = d.c0.banks[static_cast<size_t>(i)];
    std::vector<uint8_t> host(bank.bytes);
    ...
    for (int layer = 0; layer < kNumLayers; ++layer) {
      ...
      const size_t layer_offset = static_cast<size_t>(layer) * layer_bytes;
      for (size_t j = 0; j < layer_bytes; ++j) {
        state = state * 1664525U + 1013904223U;
        host[layer_offset + j] = static_cast<uint8_t>(state >> 24);
      }
    }
    bank.bo.write(host.data(), host.size(), 0);
  }
...
void run_token_round(DeviceBundle& d, StepRuns& runs, int prompt_base,
  ...
  runs.r0.set_arg(17, d.kv_k_hbm.bo);
  runs.r0.set_arg(18, d.kv_v_hbm.bo);
  runs.r0.set_arg(19, d.kv_k_t_hbm.bo);
...
int run_main(int argc, char** argv) {
  try {
    ...
    //   - write 2 fresh seed vectors plus one dummy slot into HBM[31]
    //   - launch one on-device 24-layer x 3-prompt round
    //   - slot 2 is a temporal bubble carrier: it occupies the stream slot but
    //     does not represent a useful prompt
    //   - read only the 2 useful final vectors from HBM[0] and accumulate checksums
    std::array<uint64_t, kNumPrompts> final_checksums{};
    for (int gen_round = 0; gen_round < max_new_tokens; ++gen_round) {
      std::fill(device.seed_hidden_shadow.begin(), device.seed_hidden_shadow.end(), 0);
      for (int p = 0; p < kUsefulPrompts; ++p) {
        write_seed_slot_pattern(device.seed_hidden_shadow, p, gen_round, gen_round);
      }
      device.seed_hidden_in.bo.write(device.seed_hidden_shadow.data(),
                                     device.seed_hidden_shadow.size(), 0);
      ...
      device.final_hidden_out.bo.read(device.final_hidden_shadow.data(),
                                      device.final_hidden_shadow.size(), 0);
      for (int p = 0; p < kUsefulPrompts; ++p) {
        final_checksums[static_cast<size_t>(p)] ^=
            checksum_final_slot(device.final_hidden_shadow, p);
      }
    ...
    const double total_tokens =
        static_cast<double>(kUsefulPrompts) * static_cast<double>(max_new_tokens);
    const double device_only_tok_s =
        (device_only_ms > 0.0) ? (total_tokens * 1000.0 / device_only_ms) : 0.0;
    (void)round_wall_ms;
    (void)timing_accums;
    (void)final_checksums;
    std::cout << std::fixed << std::setprecision(3)
              << device_only_tok_s << " tok/s\n";
    return 0;
