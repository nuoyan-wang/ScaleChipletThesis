...
#pragma HLS array_partition variable=arg0003 dim=3 cyclic factor=32
  for (int v0000 = 0; v0000 < 16; ++v0000) {
    ap_fixed<32, 8> v0001 =
        hls::divide((float)(ap_fixed<32, 8>)1.000000, (float)arg0001[0][v0000][0]);
...
#pragma HLS interface ap_memory port=arg0062
  ...
  dataflow_softmax_pass3_attn_v(v0021, v0022, arg0052, v0024);
...
// Persistent local KV state for 1 logical layer x 3 prompt contexts.
// This removes KV from the public top-level ABI while keeping decode-state
// local for the active layer only.
static ap_int<1> chunk0_causal_mask[1][1][1024][1024];
static fxp8_t chunk0_zero_scalar[1] = {(fxp8_t)0};
static packed_fxp32_t chunk0_kv_k[3][1][16][128][2];
static packed_fxp32_t chunk0_kv_v[3][1][16][128][2];
static packed_fxp32_t chunk0_kv_k_t[3][1][16][64][4];
...
void chunk0_stream_ip(
    ...
    ap_uint<2> prompt_id,
    ap_uint<8> cache_position,
    fxp8_t wout_bias[1024],
    packed_fxp32_t wout_weight[1024][32],
    fxp8_t qkv_bias[3072],
    packed_fxp32_t qkv_weight[1024][96],
    fxp8_t ln1_beta[1024],
    fxp8_t ln1_gamma[1024]) {
...
#pragma HLS interface s_axilite port=prompt_id bundle=control
#pragma HLS interface s_axilite port=cache_position bundle=control
#pragma HLS interface s_axilite port=return bundle=control
#pragma HLS interface ap_memory port=wout_bias
#pragma HLS interface ap_memory port=wout_weight
#pragma HLS interface ap_memory port=qkv_bias
#pragma HLS interface ap_memory port=qkv_weight
#pragma HLS interface ap_memory port=ln1_beta
#pragma HLS interface ap_memory port=ln1_gamma
#pragma HLS bind_storage variable=chunk0_kv_k type=ram_2p impl=uram
#pragma HLS bind_storage variable=chunk0_kv_v type=ram_2p impl=uram
#pragma HLS bind_storage variable=chunk0_kv_k_t type=ram_2p impl=uram

  fxp8_t hidden_local[1][1][1024];
  fxp8_t hidden_mid_local[1][1][1024];
  fxp8_t bypass_local[1][1][1024];
  fxp8_t attn_bias[1][1][1][128];
  ap_int<32> cache_position_local[1];
  ...
  cache_position_local[0] = (ap_int<32>)cache_position;

  kernel_0_chip0_core(
      chunk0_causal_mask, chunk0_zero_scalar,
      hidden_local, cache_position_local, attn_bias,
      chunk0_kv_k[prompt_id], chunk0_kv_v[prompt_id],
      hidden_mid_local, bypass_local, chunk0_kv_k_t[prompt_id],
      wout_bias, wout_weight, qkv_bias, qkv_weight, ln1_beta, ln1_gamma);
