...
void chunk2_stream_ip(
    ...
    fxp8_t fc2_bias[1024],
    packed_fxp32_t fc2_weight[4096][32]) {
#pragma HLS interface axis port=gelu_in
#pragma HLS interface axis port=bypass_in
#pragma HLS interface axis port=hidden_out
#pragma HLS interface s_axilite port=return bundle=control
#pragma HLS interface ap_memory port=fc2_bias
#pragma HLS interface ap_memory port=fc2_weight
  ...
  kernel_0_chip2_core(gelu_local, bypass_local, hidden_local, fc2_bias, fc2_weight);
