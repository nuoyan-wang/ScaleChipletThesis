//===----------------------------------------------------------------------===//
// Inline trivial const tasks (kind="const", single-op) into their consumers.
//===----------------------------------------------------------------------===//
...
namespace {
struct FuseTrivialConstsPass
    : public mlir::scale_chiplet::impl::FuseTrivialConstsPassBase<
          FuseTrivialConstsPass> {
  static bool isConstTask(TaskOp task) {
    auto attr = task->getAttrOfType<StringAttr>("kind");
    if (!attr || attr.getValue() != "const")
      return false;
  ...
  static bool pullConstsInto(TaskOp consumer) {
    bool changed = false;
    bool local = true;
    while (local) {
      local = false;
...
std::unique_ptr<mlir::Pass> mlir::scale_chiplet::createFuseTrivialConstsPass() {
  return std::make_unique<FuseTrivialConstsPass>();
}
